...
| 三阶段推进 | 按“意义 → 形态 → 判断”梳理方向，跳过已明确阶段，不重复索取确认 |
...
- 每个选项用一句话说明选择后的影响或取舍；
- 保留自由输入的“其他”入口；
- 探索性点击只作为暂定理解；明确确认或开始执行的点击有效，后续解释与修正仍然优先。

如果当前环境没有兼容控件，Skill 会展示同样的选项标签和说明，并允许用户回复序号、标签、混合方向或其他想法。仓库不捆绑自定义 GUI，因此能够在不同 Codex 环境中保持可移植性。

## GPT-6 Astra 支持

v0.3.0 按照 [OpenAI 的 Astra Skill 迁移建议](https://developers.openai.com/blog/rethinking-skills-and-prompts-for-gpt-6-astra)缩短触发描述、消除重复确认，并保留一次一问和目标共识交付。Skill 是工作流指令，不绑定模型，也不会修改 Codex 的全局模型设置；请在宿主的模型选择器中选择 **GPT-6 Astra**。其他兼容 Skill 的模型仍可使用。

| 适配点 | 实现方式 |
|---|---|
| 按宿主选控件 | 以当前模式实际允许的工具和参数为准，不因模型名称假定 GUI 存在 |
| 同步选择 | 使用获准的 `request_user_input` 等控件；仅 Plan 模式可用的工具不会在其他模式强行调用 |
| 异步选择 | 支持宿主提供的 `request_user_input_async`；发出问题后等待真实回答，工具回执和默认预选不算回答 |
| 自由输入 | 保留用户补充和混合方向；宿主已有自由输入入口时，不重复添加“其他”选项 |
| 及时修正 | 用户在过程中改口时，更新受影响的理解，不重启已经完成的梳理 |
| 及时执行 | 已确认方向或明确授权代选时，不再机械追问或重复索取开始许可 |

可点击选项来自宿主工具，并非 Astra 模型单独保证的能力。本次为指令与包装兼容更新；结构校验不等于模型行为测试，多轮交互的实机验收项目见[验收清单](tests/acceptance.md)。
